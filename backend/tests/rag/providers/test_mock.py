...
def test_no_op_retriever_returns_empty() -> None:
    r = NoOpRetriever()
    result = r.retrieve("any prompt")
    assert result == []


def test_mock_prompt_builder_returns_llm_request() -> None:
    b = MockPromptBuilder()
...
def test_mock_prompt_builder_ignores_context() -> None:
    b = MockPromptBuilder()
...
def test_mock_provider_output_validates_as_ambience_content() -> None:
    p = MockProvider()
    raw = p.generate(LLMRequest(system="", user="test"))
